...
impl Camera {
    pub fn new(location: Vec3, F: f32, target_width: u32, target_height: u32, draw_dist: f32, clip_dist: f32, frustum_inset: f32) -> Self {
        ...
        Self {transform: TransformComponent::new(location), F: F, proj_mat: pmat, target_width: target_width, target_height: target_height, aspect_ratio: aspect, pixelscale: 75., frustum_planes: frustum_planes}
    }
    ...
    pub fn project_tri(&self, tri: [Vec3; 3]) -> ([FPoint; 3], f32) {
        ...
        let frag = [p1, p2, p3];
        return (frag, (tri[0].z + tri[1].z + tri[2].z)/3.);
    }
...
impl TransformComponent {
    ...
    pub fn update_transform(&mut self) {
        ...
        let y_axis = z_axis.cross(x_axis).normalize(); // Ensure orthogonal up vector
        let corrected_x_axis = y_axis.cross(z_axis).normalize(); // Recompute x_axis for consistency

        // Rebuild the rotation matrix with normalized Vec3s extended back to Vec4
        self.rotation = Mat4::from_cols(
            corrected_x_axis.extend(0.),
            y_axis.extend(0.),
            z_axis.extend(0.),
            Vec3::ZERO.extend(1.),
        );

        assert!((self.rotation.determinant() - 1.).abs() < 0.001, "Rot matrix determinant is not 1; {}", self.rotation.determinant());
        assert!(self.rotation.inverse().abs_diff_eq(self.rotation.transpose(), 0.001), "Rot matrix inverse is not equal to transpose; i is {} but T is {}", self.rotation.inverse(), self.rotation.transpose());
...
pub fn get_points_in_triangle(tri: [FPoint; 3]) -> Vec<FPoint> {
    ...
    if max_y > min_y && max_x > min_x {
        ...
        for i in 0..=(max_y-min_y).trunc() as i32 {
            for j in 0..=(max_x-min_x).trunc() as i32 {
                ...
                let y3 = tri[2].y - min_y;
                let x1 = tri[0].x - min_x;
                let x2 = tri[1].x - min_x;
                let x3 = tri[2].x - min_x;
...
pub fn read_geometry(p: &str, vert_target: &mut Vec<Vec3>, tri_target: &mut Vec<[usize; 3]>, mat_target: &mut Vec<Material>) {
    println!("reading...");

    //Materials
    let r_file_mats: std::fs::File = std::fs::File::open(format!("{}{}", p, ".mtl")).expect("mtl file not found!");
    ...
    //Geometry
    let r_file_geo = std::fs::File::open(format!("{}{}", p, ".obj")).expect("obj file not found!");
    ...
    for line in f_geo.lines().filter_map(|result| result.ok()) {
        if !line.is_empty() {
            ...
            //Tris (and assign Mats)
                ...
                tri_target.push([is1[0].parse::<usize>().unwrap() - 1, is2[0].parse::<usize>().unwrap() - 1, is3[0].parse::<usize>().unwrap() - 1]);
                mat_target.push(*materials.get(&mat_name).unwrap());
            }
            //Loading Mats
            else {
                let segs: Vec<&str> = line.split(' ').collect();
